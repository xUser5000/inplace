...
const registerSchema = joi.object({
    ...
    password: joi.string()
        ...
        .required(),
    confirm_password: joi.equal(joi.ref('password')).messages({"any.only": "Passwords do not match"}).required()
});
...
authRouter.get('/verify/:token', async (req, res) => {
    ...
    let verificationTokenRecord = await VerificationToken.findOne({ where: { content: tokenString } });
